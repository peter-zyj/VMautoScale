...
if ($status -eq "PoweredOff") {
      ...
      if ($res -eq "y") {
         ...
         cd ./$vmm
         sed -i "/;10G-videoVlan/,/ResourceSubType/{s/E1000/E1000e/g}" ./$vmm.ovf
         #Evil Vmware,COStmp4.1.0-b42virt_disk0.vmdk COStmp4.1.0-b42virt_disk1.vmdk not work
         #Evil Vmware,COStmp4.1.0-b42virt-disk1.vmdk COStmp4.1.0-b42virt-disk2.vmdk work
         #dont know why, dont want to know why, they sucks
         sed -i "s/_disk1/-disk2/" ./$vmm.ovf
         sed -i "s/_disk0/-disk1/" ./$vmm.ovf
         
        $list = Get-ChildItem | where {$_.extension -eq ".vmdk"} 
        $name1 = $list[0].name
        $name2 = $list[1].name

        $newName1 = $name1 -replace "_disk0","-disk1"
        $newName2 = $name2 -replace "_disk1","-disk2"
